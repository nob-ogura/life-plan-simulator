...
- 自動テスト方針: 不要（Task 10 の E2E で骨格表示を担保できるため、個別の自動テストは省略可）。
- 受入基準（Gherkin）:
...
- 自動テスト方針: 要（必須・数値・年月の代表バリデーションをユニット/統合テストで固定化する）。
- 受入基準（Gherkin）:
  - シナリオ: 必須項目未入力で保存できない
    - 前提: 必須項目が未入力である
    - もし: 保存ボタンを押す
...
- 自動テスト方針: 要（本人/配偶者必須、子どもは birth/due のいずれか必須のドメインルールを自動化する）。
- 受入基準（Gherkin）:
...
- 自動テスト方針: 要（保存→再取得のデータ整合性が重要なため、統合/E2E で確認する）。
- 受入基準（Gherkin）:
  - シナリオ: 収入とボーナスが保存できる
...
- 自動テスト方針: 要（`retirement_bonus` 排他ルールと繰り返しイベント登録を自動化する）。
- 受入基準（Gherkin）:
...
- 自動テスト方針: 要（`retirement_bonus` が常に 1 件のみで更新になることを自動化する）。
- 受入基準（Gherkin）:
...
- 自動テスト方針: 要（保存→再取得で設定値が保持されることを自動化する）。
- 受入基準（Gherkin）:
...
- 自動テスト方針: 要（上書き保存で最新のみ保持され、履歴が増えないことを自動化する）。
- 受入基準（Gherkin）:

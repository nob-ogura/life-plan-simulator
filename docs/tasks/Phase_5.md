...
- 自動テスト: 不要（Task 10 の E2E で骨格表示を担保できるため、個別の自動テストは省略可）。
- 受入基準（Gherkin）:
...
- 自動テスト: 要（必須・数値・年月の代表バリデーションをユニット/統合テストで固定化する）。
- 受入基準（Gherkin）:
  - シナリオ: 必須項目未入力で保存できない
    - 前提: 必須項目が未入力である
    - もし: 保存ボタンを押す
...
- 自動テスト: 要（本人/配偶者必須、子どもは birth/due のいずれか必須のドメインルールを自動化する）。
- 受入基準（Gherkin）:
...
- 作業内容:
  - 収入フォームで手取り月額、昇給率、期間を入力可能にする。
  - ボーナスフォームで定期収入に紐づくボーナス月/金額/変化点を入力可能にする。
  - 支出フォームで月額、インフレ率、期間、カテゴリを入力可能にする。
  - 収入/支出の追加・編集・削除を行い、保存時に Supabase へ upsert する。
- 自動テスト: 要（保存→再取得のデータ整合性が重要なため、統合/E2E で確認する）。
- 受入基準（Gherkin）:
  - シナリオ: 収入とボーナスが保存できる
...
- 作業内容:
  ...
  - 一覧からライフイベントを削除できるようにする。
  - `retirement_bonus` は汎用イベントから新規追加不可とし、既存編集時は専用フォームへ誘導または警告を表示する。
- 自動テスト: 要（`retirement_bonus` 排他ルールと繰り返しイベント登録を自動化する）。
- 受入基準（Gherkin）:
...
- 自動テスト: 要（`retirement_bonus` が常に 1 件のみで更新になることを自動化する）。
- 受入基準（Gherkin）:
...
- 自動テスト: 要（保存→再取得で設定値が保持されることを自動化する）。
- 受入基準（Gherkin）:
...
- 自動テスト: 要（上書き保存で最新のみ保持され、履歴が増えないことを自動化する）。
- 受入基準（Gherkin）:

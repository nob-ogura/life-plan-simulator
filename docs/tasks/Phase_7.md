...
- 目的: UI と計算結果が一致していることをエンドツーエンドで保証する（既存E2Eの安定性を優先）。
- 作業内容:
  - **E2E専用 Seed API** を用意し、テスト開始時に必要データを一括投入する。
    - `/__e2e/seed` で profile / simulation_settings / rentals / life_events を作成。
    - 住宅購入イベントは `auto_toggle_key=HOUSING_PURCHASE_STOP_RENT` をSeed時に付与する。
    - Seed API の返却値で「開始年月」「検証対象年月（停止月/繰り返し月）」を返す。
  - Playwright では UI 入力を最小限にし、**ダッシュボードの月次表・サマリ** で結果のみを検証する。
  - 既存のE2Eに影響しないよう、Seed API はテスト専用パスで実装する。
- 自動テスト: 要（E2E: `tests/e2e` にシナリオを追加する）。
- 受入基準（Gherkin）:
  - シナリオ1: 住宅購入で家賃が購入月の前月に停止する
    - 前提: E2E Seed で賃貸の開始月が投入済み
    - かつ: E2E Seed で住宅購入イベントが登録されている
    - もし: ダッシュボードを表示する
    - ならば: 家賃の計上が住宅購入月の前月で停止している
  - シナリオ2: 繰り返しイベントが停止条件で打ち切られる
    - 前提: E2E Seed で繰り返しイベントに停止条件（回数または年齢）が設定されている
    - もし: ダッシュボードを表示する
